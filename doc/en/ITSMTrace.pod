...
service or CI at that level, but not beyond. B<ITSMTrace> can show I<all> 
...
Please see the perldoc of C<Kernel::System::ITSMTrace> for details about
option names and accepted values. 


=head2 Data Model

In the context for which B<ITSMTrace> has initially been developed, there
...
B<ITSMTrace> wraps services and CIs as "objects" with a common interface, 
...
is used, i.e. B<ITSMTrace> only knows about C<DependsOn> but not about 
...
B<ITSMTrace> is integrated with the web UI of OTRS in the areas
...
B<ITSMTrace> also supplies a low-level command line interface 
...
pod2man ITSMTrace.pod > ITSMTrace.man; \
groff -man ITSMTrace.man > ITSMTrace.ps; \
gs -dCompatibilityLevel=2 -dNOPAUSE -dBATCH \
 -dAutoRotatePages=/None -sDEVICE=pdfwrite \
 -sOutputFile=ITSMTrace.pdf ITSMTrace.ps 
########################################################################
